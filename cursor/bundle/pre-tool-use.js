...
var log3 = (msg) => log("cursor-pre-tool-use", msg);
async function main() {
  ...
  if (!config) {
    log3("no config \u2014 falling through to Cursor's bash");
    return;
  }
  ...
  try {
    ...
    if (result === null) {
      log3(`fallthrough \u2014 handleGrepDirect returned null for "${grepParams.pattern}"`);
      return;
    }
