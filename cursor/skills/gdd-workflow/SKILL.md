---
name: gdd-workflow
description: Use for Goal-Driven Development (GDD) when running experiments, comparing implementation variants, or coordinating with other agents on measurable project changes. Always inspect shared memory for prior related work and log every experimental outcome through the current hivemind gdd command.
allowed-tools: Bash
---
...
Use the installed Cursor plugin GDD command:

```bash
node "$HOME/.cursor/hivemind/bundle/commands/gdd.js" log <change_id> <metric> <status> "<description>" [--metadata <json>] [--metadata-file <path>] [--table <name>]
```

If the installed plugin path is known, run the same command implementation through the plugin bundle:
...
```bash
node "$HOME/.cursor/hivemind/bundle/commands/gdd.js" promote <change_id> [--id <uuid>] [--unset] [--table <name>]
```
...
```bash
node "$HOME/.cursor/hivemind/bundle/commands/gdd.js" init [--table <name>]
```
