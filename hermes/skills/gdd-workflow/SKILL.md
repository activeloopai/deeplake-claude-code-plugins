...
Use the installed Hermes plugin GDD command:

```bash
node "$HOME/.hermes/hivemind/bundle/commands/gdd.js" log <change_id> <metric> <status> "<description>" [--metadata <json>] [--metadata-file <path>] [--table <name>]
```
...
```bash
node "$HOME/.hermes/hivemind/bundle/commands/gdd.js" promote <change_id> [--id <uuid>] [--unset] [--table <name>]
```
...
```bash
node "$HOME/.hermes/hivemind/bundle/commands/gdd.js" init [--table <name>]
```
