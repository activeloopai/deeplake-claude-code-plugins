...
export class DeeplakeApi {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        lastError = e instanceof Error
          ? new DeeplakeQueryError(e.message, { sql, cause: e })
          : new DeeplakeQueryError(String(e), { sql, cause: e });
  ...
  async ensureSessionsTable(name: string): Promise<void> {
    ...
    for (const [column, ddl] of alterColumns) {
      try {
        ...
        // Some backends may not support ADD COLUMN IF NOT EXISTS; keep going so older tables still work.
      }
    }
    await this.ensureLookupIndex(name, "path_creation_date_turn_index", `("path", "creation_date", "turn_index")`);
  }

  async ensureGraphNodesTable(name: string): Promise<void> {
    const columns = [
      `id TEXT NOT NULL DEFAULT ''`,
      `path TEXT NOT NULL DEFAULT ''`,
      `filename TEXT NOT NULL DEFAULT ''`,
    ...
    for (const [column, ddl] of [
      ...
      try {
        ...
        // Best effort for older backends.
      }
    }
  ...
  async ensureGraphEdgesTable(name: string): Promise<void> {
    const columns = [
      `id TEXT NOT NULL DEFAULT ''`,
      `path TEXT NOT NULL DEFAULT ''`,
      `filename TEXT NOT NULL DEFAULT ''`,
  ...
  async ensureFactsTable(name: string): Promise<void> {
    const columns = [
      `id TEXT NOT NULL DEFAULT ''`,
      `path TEXT NOT NULL DEFAULT ''`,
      `filename TEXT NOT NULL DEFAULT ''`,
  ...
  async ensureEntitiesTable(name: string): Promise<void> {
    const columns = [
      `id TEXT NOT NULL DEFAULT ''`,
      `path TEXT NOT NULL DEFAULT ''`,
      `filename TEXT NOT NULL DEFAULT ''`,
  ...
  async ensureFactEntityLinksTable(name: string): Promise<void> {
    const columns = [
      `id TEXT NOT NULL DEFAULT ''`,
      `path TEXT NOT NULL DEFAULT ''`,
      `filename TEXT NOT NULL DEFAULT ''`,
